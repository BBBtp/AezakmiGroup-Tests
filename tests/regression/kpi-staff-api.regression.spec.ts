...
test.describe('KPI staff service', () => {
    ...
    test('Карточка ASO manager загружает все KPI endpoint', async ({ kpiPage }) => {
        ...
        const responses = expectedEndpoints.map(([, path]) => kpiPage.page.waitForResponse((response) => response.url().includes(path)));

        await loggedAction(kpiPage.page, 'navigate', `KPI manager ${employeeId}`, kpiPage.page.locator('body'), () => kpiPage.page.goto(`/kpi/${employeeId}`, { waitUntil: 'domcontentloaded' }));
        const endpointResponses = await Promise.all(responses);
        await Promise.all(endpointResponses.map((response, index) => expectSuccessfulJson(response, expectedEndpoints[index][0])));

        const vacationsResponse = kpiPage.page.waitForResponse((response) => response.url().includes(managerKpiPath(employeeId, 'vacations/history')));
        ...
        await expect(kpiPage.page.getByText('Impact of employee vacation on minimum rating.', { exact: true })).toBeVisible();
    ...
    test('Settings отображает стартовый балл текущего месяца из API', async ({ kpiPage }) => {
        ...
        for (const candidate of candidates) {
            const vacationsResponse = kpiPage.page.waitForResponse((response) => response.url().includes(managerKpiPath(candidate.employee_id, 'vacations/history')));
            await loggedAction(kpiPage.page, 'navigate', `KPI settings for ${candidate.employee_id}`, kpiPage.page.locator('body'), () => kpiPage.page.goto(`/kpi/${candidate.employee_id}/settings`, { waitUntil: 'domcontentloaded' }));
            await expectSuccessfulJson(await vacationsResponse, 'vacations history');

            const rows = kpiPage.page.getByRole('row');
            const hasDataRows = await expect.poll(
                () => rows.count(),
    ...
    test('Ошибка vacations history показывает error-state страницы Settings сотрудника', async ({ kpiPage }) => {
