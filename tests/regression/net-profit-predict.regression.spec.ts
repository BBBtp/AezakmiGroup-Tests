...
test.describe('ASA → Net profit predict', () => {
  ...
  test('скрывает будущие месяцы только для текущего года', async ({ netProfitPredictPage }) => {
  ...
  test('сохраняет App-фильтр и период в searchParams после reload', async ({
    ...
    expect(await netProfitPredictPage.content.searchParams()).toMatchObject(params);
  });

  test('показывает раскрытую строку фильтров с закреплённым App: All', async ({ netProfitPredictPage }) => {
  ...
  test('сворачивает и повторно раскрывает строку фильтров', async ({ netProfitPredictPage }) => {
    await allure.allureId('996');
    await netProfitPredictPage.openFromSidebar(new Date());
    await netProfitPredictPage.content.expectFilterRowToggle();
  });

  test('выбирает год и применяет произвольный период из календаря', async ({ netProfitPredictPage }) => {
  ...
  test('применяет значения фильтра App status', async ({ netProfitPredictPage }) => {
    await allure.allureId('998');
    await netProfitPredictPage.openFromSidebar(new Date());
    await netProfitPredictPage.content.expectAppStatusFilter();
  });

  test('поддерживает Select all, поиск и частичный выбор App', async ({ netProfitPredictPage }) => {
    await allure.allureId('999');
    await netProfitPredictPage.openFromSidebar(new Date());
    await netProfitPredictPage.content.selectSingleApp();
  });

  test('фильтрует данные по Team и сохраняет выбор после reload', async ({
    netProfitPredictPage,
    network,
  }) => {
  ...
  test('показывает пустое состояние для несовместимых фильтров и сбрасывает их', async ({
    netProfitPredictPage,
  }) => {
