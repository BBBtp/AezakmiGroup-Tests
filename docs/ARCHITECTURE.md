...
компонентах. Например, `locators/kpi.ts` и `locators/kpi-settings.ts` являются единственными
источниками статических и динамических test ID/CSS-контрактов для KPI, графика, фильтров, карточек,
сотрудников и настроек. Компоненты получают готовые значения из контракта и создают `Locator`
через `LocatorFactory`. Приоритет: `data-testid`, затем role/label, затем документированный
устойчивый CSS.
...
Архитектурная проверка не позволяет KPI UI возвращаться к строковым test ID/CSS и raw `expect()`
внутри UI-объектов; следующие домены переводятся на такой контракт отдельными срезами.
