...
компонентах. Например, `locators/kpi-settings.ts` является единственным источником статических и
динамических test ID для страницы, таблиц, форм и строк KPI Settings. Компоненты получают готовые
значения из контракта и создают `Locator` через `LocatorFactory`. Приоритет: `data-testid`, затем
role/label, затем документированный устойчивый CSS.
...
Архитектурная проверка не позволяет KPI Settings возвращаться к строковым test ID и raw
`expect()` внутри UI-объектов; следующие домены переводятся на такой контракт отдельными срезами.
