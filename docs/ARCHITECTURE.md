...
и модалками Comment/Technical task. `AbTestCreateComponent` проверяет локальное поведение initial
file upload без отправки формы. Контролируемые ответы Apps и A/B tests находятся в
`tests/support/product`.
Settings-сценарии Parameters проходят через `AdministrationPage` и `AdministrationComponent`;
редактирование, удаление и добавление параметров проверяются через отмену диалога и отсутствие
незапланированной API-мутации. Список Staff/Employees опубликован отдельным `EmployeesPage`:
`EmployeesListComponent` владеет таблицей, сортировкой, пагинацией, поиском и системными
состояниями, а контракт списка находится в `tests/support/employees`.
...
`doqa-client.mjs` работает с API DoQA: ищет кандидатов, читает кейсы и чек-листы, создаёт
чек-листы, безопасно добавляет в них новые пункты, переводит кейс в работу, загружает отчёты и
формирует read-only черновики дефектов элементов прогона. PATCH использует ETag той же версии, которая была прочитана и
проанализирована; при `412 Precondition Failed` клиент повторно читает кейс и не перезаписывает
чужое изменение. Обновление чек-листа не заменяет и не удаляет существующие пункты и завершается
контрольным чтением. Bug-draft flow читает run element, выполняет дедупликацию активных багов по
