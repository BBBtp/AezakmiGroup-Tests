...
export async function loggedAction<T>(
    ...
    try {
        ...
        const actual = await locator.count().then(count => `count=${count}`).catch(() => 'count=<unavailable>');
        write('FAIL', `${context(page)} action="${action}" target="${target}" locator="${locatorText}" ${actual} error="${error instanceof Error ? error.message : String(error)}"`);
        throw error;
    }
}
...
export async function loggedExpectation(
    ...
    write('EXPECT', `${context(page)} target="${target}" locator="${safeLocator(locator)}" expected="${expectation}"`);
    try {
        await operation();
    } catch (error) {
        const state = await locator.count().then(async count => {
            const visible = count > 0 ? await locator.first().isVisible().catch(() => false) : false;
            return `count=${count} visible=${visible}`;
        }).catch(() => 'state=<unavailable>');
        write('FAIL', `${context(page)} target="${target}" locator="${safeLocator(locator)}" ${state} error="${error instanceof Error ? error.message : String(error)}"`);
        throw error;
    }
}

export function loggedClick(page: Page, target: string, locator: Locator, options?: Parameters<Locator['click']>[0]): Promise<void> {
...
export function loggedSelectOption(page: Page, target: string, locator: Locator, options: Parameters<Locator['selectOption']>[0]): Promise<string[]> {
