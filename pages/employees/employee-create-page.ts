...
export class EmployeeCreatePage extends BasePage {
    ...
    async openWorkingInfo(): Promise<void> {
        ...
        await loggedAction(this.page, 'press', 'employee personal info: close date picker', this.birthDateInput, () => this.birthDateInput.press('Escape'));
    ...
    async selectAsoManagerPosition(): Promise<void> {
        ...
        await loggedClick(this.page, 'employee working info: module ASO', this.page.getByRole('option', { name: 'ASO', exact: true }));

        await loggedClick(this.page, 'employee working info: department selector', this.departmentSelect);
        await loggedClick(this.page, 'employee working info: department ASA', this.page.getByRole('option', { name: 'ASA', exact: true }));

        await loggedClick(this.page, 'employee working info: direction selector', this.directionSelect);
        await loggedClick(this.page, 'employee working info: direction Product', this.page.getByRole('option', { name: 'Product', exact: true }));
